...
BEGIN
	...
	CREATE TABLE #HumandProcesar (
        ...
        OUTPUT  inserted.id, inserted.employeeId, inserted.tipoMarcaje, inserted.clasificacion, inserted.Fecha, inserted.minutosDesviacion
            INTO #HumandProcesar (id, employeeId, tipoMarcaje, clasificacion, Fecha, minutosDesviacion)
        FROM [Humand].[dbo].[Humand] a
        JOIN Personal b ON a.employeeId = b.Personal
        WHERE (
                ISNULL(a.clasificacion, '') IN ('RETARDO_MENOR', 'RETRASO_COMIDA_MENOR', 'RETARDO_MAYOR', 'RETRASO_COMIDA_MAYOR','FALTA','PERMISO_HORAS')
                OR (    UPPER(LTRIM(RTRIM(ISNULL(b.Sindicato, '')))) = 'SINDICALIZADO'
                    AND a.tipoMarcaje IN ('ENTRADA', 'REGRESO_COMIDA')
                    AND a.minutosDesviacion > @ToleranciaMin
                    AND ISNULL(a.clasificacion, '') NOT LIKE 'REVISAR_HORARIO%'
                    AND ISNULL(a.clasificacion, '') <> 'JUSTIFICADO')
              )
        ...
        */
        ;WITH FechasIncidencia AS (
            ...
            WHERE i.TipoIncidencia IN (
                'PERMISO_DEFUNCION_FAM_DIRECTO',
                'PERMISO_DEFUNCION_FAM_INDIRECTO',
                'PERMISO_PATERNIDAD',
    ...
    SELECT
		...
		b.Departamento,
		b.SucursalTrabajo,
        SUM(CASE WHEN s.EsSindicalizado = 0 AND a.tipoMarcaje = 'ENTRADA'        AND a.clasificacion = 'RETARDO_MENOR'          THEN 1 ELSE 0 END) AS RetardoMenorEntrada, --RETARDO_MENOR
        SUM(CASE WHEN s.EsSindicalizado = 0 AND a.tipoMarcaje = 'REGRESO_COMIDA' AND a.clasificacion = 'RETRASO_COMIDA_MENOR'    THEN 1 ELSE 0 END) AS RetardoMenorComida, --RETRASO_COMIDA_MENOR
        SUM(CASE WHEN s.EsSindicalizado = 0 AND a.tipoMarcaje = 'ENTRADA'        AND a.clasificacion = 'RETARDO_MAYOR'           THEN 1 ELSE 0 END) AS RetardoMayorEntrada,  ----RETARDO_MAYOR
        SUM(CASE WHEN s.EsSindicalizado = 0 AND a.tipoMarcaje = 'REGRESO_COMIDA' AND a.clasificacion = 'RETRASO_COMIDA_MAYOR'     THEN 1 ELSE 0 END) AS RetardoMayorComida, --RETRASO_COMIDA_MAYOR
		SUM(CASE WHEN a.tipoMarcaje = 'FALTA'          AND a.clasificacion = 'FALTA'        THEN 1 ELSE 0 END) AS Falta, --FALTA
		SUM(CASE WHEN s.EsSindicalizado = 0 AND a.tipoMarcaje IN ('ENTRADA','REGRESO_COMIDA') AND a.clasificacion = 'PERMISO_HORAS'        THEN 1 ELSE 0 END) AS PermisoHoras, --ENTRADA,REGRESO_COMIDA
		CAST(SUM(CASE WHEN s.EsSindicalizado = 1
		               AND a.tipoMarcaje IN ('ENTRADA','REGRESO_COMIDA')
		               AND a.minutosDesviacion > @ToleranciaMin
		               AND ISNULL(a.clasificacion, '') NOT LIKE 'REVISAR_HORARIO%'
		              THEN CEILING((a.minutosDesviacion - @ToleranciaMin) / CAST(@BloqueMin AS decimal(9,2)))
		              ELSE 0 END) AS int) AS UnidadesSindicalizado,
        SUM(CASE WHEN a.clasificacion = 'PERMISO_DEFUNCION_FAM_DIRECTO'   THEN 1 ELSE 0 END) AS PermisoDefuncionFamDirecto,
        SUM(CASE WHEN a.clasificacion = 'PERMISO_DEFUNCION_FAM_INDIRECTO' THEN 1 ELSE 0 END) AS PermisoDefuncionFamIndirecto,
	...
	JOIN Personal b ON a.employeeId = b.Personal
	CROSS APPLY (SELECT EsSindicalizado = CASE WHEN UPPER(LTRIM(RTRIM(ISNULL(b.Sindicato, '')))) = 'SINDICALIZADO' THEN 1 ELSE 0 END) s
   GROUP BY b.Departamento,b.SucursalTrabajo,b.Categoria,b.Personal, b.PeriodoTipo, b.Empresa, a.Fecha
    ...
    INTO #Detalle
    FROM #Incidencias
	UNPIVOT (Valor FOR TipoIncidencia IN (RetardoMenorEntrada, RetardoMenorComida, RetardoMayorEntrada,RetardoMayorComida,Falta,PermisoHoras,PermisoDefuncionFamDirecto,PermisoDefuncionFamIndirecto,PermisoPaternidad,TiempoPorTiempoHoras,Vacaciones,IncapacidadEnfermedad,IncapacidadAccidente,UnidadesSindicalizado)) AS u
    /* ---------------------------------------------------------------------
    ...
    SELECT * INTO #Reglas
    FROM (VALUES
        ('RetardoMenorEntrada',					'RetardoMenorEntrada',					'RetardoMenorEntrada',                  3,				1,			0),
        ('RetardoMenorComida',					'Retardos Menores',						'Retardos Menores Comida Acumulados',	3,				1,			0),
        ('RetardoMayorEntrada',					'Retardos Mayores',						'Retardos Mayor Entrada',				1,				0,			0),
		('RetardoMayorComida',					'Retardos Mayores',						'Retardos Mayor Entrada Comida',		1,				0,			0),
		('Falta',								'Falta Injustificada',			        'Falta',								1,				0,			0),
		('Falta',								'Falta Justificada',			        'Falta',								1,				0,			0),
		('PermisoHoras',						'Permiso Horas',						'Permiso Horas',						1,				0,			0),
        ('PermisoDefuncionFamDirecto',			'Permiso Defuncion',					'Permiso Defunción Fam. Directo',		1,				0,			0),
        ('PermisoDefuncionFamIndirecto',		'Permiso Defuncion',					'Permiso Defunción Fam. Indirecto',		1,				0,			0),
        ('PermisoPaternidad',					'Permiso Nacimiento',					'Permiso Paternidad',					1,				0,			0),
        ('TiempoPorTiempoHoras',				'Permiso Horas',						'Tiempo por Tiempo Horas',				1,				0,			0),
        ('Vacaciones',							'.',									'Vacaciones',							1,				0,			0),
        ('IncapacidadEnfermedad',				'Enfermedad General Inicial',			'Incapacidad Enfermedad',				1,				0,			0),
        ('IncapacidadAccidente',				'Enfermedad General Inicial',			'Incapacidad Accidente',				1,				0,			0),
        ('UnidadesSindicalizado',				'Permiso Horas',						'Unidades Retardo Sindicalizados',		1,				0,			1)	
    ) AS R(TipoIncidencia,							Concepto,								ObservacionBase,				UmbralBase, UsaDivisorSancion, AcumulaPeriodo)

    SELECT
        r.TipoIncidencia, r.Concepto, r.ObservacionBase, r.UmbralBase, r.UsaDivisorSancion, r.AcumulaPeriodo,
        p.PeriodoTipo,
		s.Sucursal,
        PeriodoValor = @Semana,
    ...
    DECLARE @Total int = (SELECT COUNT(*) FROM #Combinaciones), @i int = 1
    DECLARE
        @Id					int,
    ...
    BEGIN
        ...
        IF @TipoIncidencia <> 'RetardoMayorEntrada'
        AND	EXISTS (
            SELECT 1
            FROM #Detalle d
        ...
        BEGIN
            SELECT @Observaciones = @ObservacionBase + ' '
				 ...
				 @Mov = CASE @TipoIncidencia WHEN 'Falta'							THEN 'Faltas' 
											 ...
											 WHEN 'UnidadesSindicalizado'			THEN 'Prestacion'	 
											 ELSE 'Prestacion' END
            ...
            SELECT
                ...
                SUM(CASE WHEN @UsaDivisorSancion = 1
                         ...
                         ELSE d.Valor END),
                CASE WHEN @AcumulaPeriodo = 1 THEN 'Semana ' + @PeriodoValor ELSE CONVERT(varchar(50), d.Fecha) END,
                @Fecha,
                1, 0, 0
            FROM #Detalle d
            JOIN #Periodos p ON p.PeriodoTipo = d.PeriodoTipo
	...
	BEGIN CATCH
        IF XACT_STATE() <> 0
            ROLLBACK TRAN 
        SELECT 'Error al procesar incidencias: ' + ERROR_MESSAGE()
	END CATCH
    RETURN
END
GO