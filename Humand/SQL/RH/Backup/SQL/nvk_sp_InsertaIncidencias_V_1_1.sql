...
BEGIN
    ...
    SELECT
        ...
        @Quincena  = CASE WHEN ISNULL(@Quincena,'') = '' THEN ((MONTH(@Fecha) - 1) * 2 + CASE WHEN DAY(@Fecha) <= 15 THEN 1 ELSE 2 END - 1)
                          ELSE @Quincena END
    ...
    SELECT 'Semanal',
        ...
        FechaDCorte, FechaACorte, AplicaSancionEspecial
    ...
    WHERE ISNULL(clasificacion, '') IN ('RETARDO_MENOR', 'RETRASO_COMIDA_MENOR', 'RETARDO_MAYOR', 'RETRASO_COMIDA_MAYOR',/*'SALIDA_ANTICIPADA',*/'FALTA','PERMISO_HORAS'/*,'REVISAR_HORARIO'*/)
      AND ISNULL(tipoIncidenciaAplicada, '') = ''
      AND b.Estatus IN ('ALTA')
      AND b.PeriodoTipo IN ('Semanal', 'Quincenal')
    ...
    SELECT
        ...
        p.PeriodoTipo,
		s.Sucursal,
        PeriodoValor = CASE p.PeriodoTipo WHEN 'Semanal' THEN @Semana ELSE @Quincena END,
        Rn = ROW_NUMBER() OVER (ORDER BY r.TipoIncidencia, p.PeriodoTipo)
    INTO #Combinaciones
    FROM #Reglas r
    CROSS JOIN (SELECT DISTINCT PeriodoTipo FROM #Periodos WHERE FechaPago = @UltimoViernes) p
