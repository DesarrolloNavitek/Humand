...
BEGIN
    ...
    /* ---------------------------------------------------------------------
       ...
       --------------------------------------------------------------------- */
    SELECT id,Personal, PeriodoTipo, Fecha, TipoIncidencia,SucursalTrabajo,Categoria, Valor
    INTO #Detalle
    FROM #Incidencias
    ...
    INTO #Combinaciones
    FROM #Reglas r
    CROSS JOIN (SELECT DISTINCT PeriodoTipo FROM #Periodos) p
