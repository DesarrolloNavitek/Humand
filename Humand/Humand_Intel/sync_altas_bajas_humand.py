"""
Sincroniza altas y bajas de NAVILUX (Intelisis, base real) hacia Humand:

  1. ALTAS: empleados activos en NAVILUX.Personal que no tienen cuenta en
     Humand -> se crean.
  2. BAJAS (deshabilitar): empleados con Estatus='BAJA' en NAVILUX que
...
DB_NAME = os.environ["DB_NAME"]  # NVTEST -- consulta NAVILUX cruzado
...
def obtener_bajas_navilux(cursor):
    cursor.execute(
        """
    ...
    for row in cursor.fetchall():
        clave = str(row.Personal).strip().zfill(6)
        empleados[clave] = f"{row.Nombre} {row.ApellidoPaterno} {row.ApellidoMaterno or ''}".strip()
    return empleados
...
def procesar_altas(activos_navilux, usuarios_humand, confirmar):
    faltantes = sorted(set(activos_navilux) - set(usuarios_humand))
    print(f"\n=== ALTAS: {len(faltantes)} empleado(s) en NAVILUX sin cuenta en Humand ===")
    ...
    for clave in faltantes:
        ...
        payload = {
            ...
            "segmentation": [{"group": "Razón Social", "item": RAZON_SOCIAL_OBJETIVO}],
        ...
        if d["departamento"]:
            ...
            print(f"  [AVISO] {clave} {d['nombre']} -- sin Departamento en NAVILUX, se crea sin esa segmentación.")

        if d["reportaA"]:
            payload["relationships"] = [{"name": "BOSS", "employeeInternalId": d["reportaA"]}]
        else:
            print(f"  [AVISO] {clave} {d['nombre']} -- sin ReportaA en NAVILUX, se crea sin jefe asignado.")
...
def procesar_bajas_deshabilitar(cursor, conn, bajas_navilux, usuarios_humand, confirmar):
    ...
    for clave in a_deshabilitar:
        ...
        if resp.status_code == 204:
            ...
            cursor.execute(
                """
                MERGE dbo.HumandBajasPendientes AS destino
...
def procesar_eliminaciones(cursor, conn, confirmar):
    cursor.execute(
        ...
        FROM dbo.HumandBajasPendientes
    ...
    for row in pendientes:
        ...
        if resp.status_code == 204:
            cursor.execute(
                "UPDATE dbo.HumandBajasPendientes SET eliminado = 1, fechaEliminado = GETDATE() WHERE employeeInternalId = ?",
                clave,
            )
            conn.commit()
...
def procesar_barrido_segmentaciones(activos_navilux, usuarios_humand, confirmar):
    ...
    Departamento/Puesto actual en NAVILUX contra lo que Humand tiene
...
def main(confirmar, solo_altas, solo_bajas, solo_eliminar, solo_barrido):
    ...
    if solo_altas or correr_todo:
        activos_navilux = obtener_activos_navilux(cursor)
        usuarios_humand = obtener_usuarios_humand()
        procesar_altas(activos_navilux, usuarios_humand, confirmar)

    if solo_bajas or correr_todo:
        bajas_navilux = obtener_bajas_navilux(cursor)
        usuarios_humand = obtener_usuarios_humand()
