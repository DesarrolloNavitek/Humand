...
retardo menor/mayor, salida anticipada, comida por duración e
incidencias/justificaciones).

Flujo:
    1. Lee la última fecha de corrida exitosa (tabla HumandSyncControl).
    2. Trae todos los empleados disponibles para la API Key de Humand.
    ...
    5. Clasifica cada marcaje del día según corresponda:
        ...
        - Si no hay incidencia -> ENTRADA / SALIDA_COMIDA / REGRESO_COMIDA / SALIDA,
...
# =========================================================
# 2. Empleados: Humand (GET /users) + incidencias
# =========================================================
...
def obtener_empleados_humand() -> list:
    ...
    log.info(f"Empleados encontrados en Humand (todas las comunidades): {len(empleados)}.")
    return empleados
...
#JARC: Se actualizadon las incidencias configuradas en Humand
MAPA_POLICY_A_TIPO_INTERNO = {
    ...
    "Permiso con goce de Salario": "PERMISO_CON_GOCE",
    "Permiso Sin Goce de Salario": "PERMISO_SIN_GOCE",
    "Permiso Onomástico": "PERMISO_CUMPLEAÑOS",
    "Permiso por defunción": "PERMISO_DEFUNCION_FAM_DIRECTO",
    "Permiso por defunción (Familiar Indirecto)": "PERMISO_DEFUNCION_FAM_INDIRECTO",
    "Permiso por Paternidad": "PERMISO_PATERNIDAD",
    "Tiempo por tiempo horas": "TIEMPO_POR_TIEMPO_HORAS",
...
def clasificar_entrada_o_regreso(hora_real: datetime.datetime, hora_esperada: datetime.datetime):
    ...
    if diferencia_min <= LIMITE_RETARDO_MENOR:
...
def clasificar_comida_duracion(hora_salida_comer: datetime.datetime, hora_regreso: datetime.datetime):
    ...
    if exceso_min <= LIMITE_RETARDO_MENOR:
    ...
    for idx, entry in enumerate(entries_ordenadas):
        ...
        if tipo_marcaje == "ENTRADA":
            if horario_in:
                ...
                clasificacion, minutos_desviacion = clasificar_entrada_o_regreso(hora_real, hora_esperada)

        elif tipo_marcaje == "REGRESO_COMIDA":
            hora_salida_comer = parsear_hora_utc(entries_ordenadas[idx - 1]["time"])
            clasificacion, minutos_desviacion = clasificar_comida_duracion(hora_salida_comer, hora_real)
            if alerta_dia:
...
def clasificar_marcajes_del_dia(entries: list, time_slots: list, fecha_referencia: datetime.date,
                                 employee_internal_id: str, tipo_incidencia: str = None,
                                 horario_historico: dict = None, catalogo_turnos: list = None,
                                 catalogo_rotativo: dict = None):
    ...
    return clasificar_oficina(entries_ordenadas, time_slots, fecha_referencia, employee_internal_id,
                               horario_historico, catalogo_turnos, catalogo_rotativo)
...
def insertar_marcaje(cursor, conn, fecha: datetime.date, nombre_colaborador: str, registro: dict):
    try:
        cursor.execute(
            """
        ...
        for fecha_ini, fecha_fin_incidencia, tipo in incidencias:
            ...
            while fecha_actual <= fecha_fin_efectiva:
                ...
                if not ya_existe:
                    ...
                    insertar_marcaje(cursor, conn, fecha_actual, nombre_colaborador, registro)
                    total_asegurados += 1
                    log.info(
                ...
                for resumen in resumenes:
                    ...
                    try:
                        ...
                        if not entries:
                            if tipo_incidencia:
                            ...
                            elif es_dia_laboral and not es_especial:
                                ...
                                continue  # descanso o día especial (home office, etc.): no se inserta nada
                        else:
                            ...
                            registros_clasificados = clasificar_marcajes_del_dia(
                                ...
                                catalogo_rotativo=catalogo_rotativo,
                            )
                    except Exception as e:
                    ...
                    for registro in registros_clasificados:
                        ...
                        insertado = insertar_marcaje(cursor, conn, fecha_ref, nombre_colaborador, registro)
                        if insertado:
                            total_insertados += 1
                        else:
                            total_omitidos += 1
